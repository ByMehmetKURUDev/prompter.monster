...
import { Brain, Gauge, Star } from "lucide-react";
...
export function Step4Experts({
  ...
  s: StudioState;
  toggle: (key: "experts", v: string) => void;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <StepHeader icon="👹" title="Uzmanlar & Üret" subtitle="12 canavar uzman, 3-5 seç, mega prompt üret" />
      ...
      <Card className="p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          >
            {s.experts.length} seçili • 3-5 önerilir
          </span>
        </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <Card className="p-4 md:col-span-2">
          <SectionLabel className="mb-2 text-zinc-500">FORMAT</SectionLabel>
          <div className="flex flex-wrap gap-2">
            {FORMATS.map((f) => (
              <button
                key={f}
                type="button"
                onClick={() => patch({ format: f as OutputFormat })}
                aria-pressed={s.format === f}
                className={cx(
                  "px-3 h-9 rounded-xl border text-[12px]",
