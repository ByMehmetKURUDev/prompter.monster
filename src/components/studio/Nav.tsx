...
export function Nav({
  ...
  return (
    <nav className="h-[56px] border-b border-ink-600 bg-ink-950/80 backdrop-blur-xl sticky top-0 z-50 flex items-center px-4 lg:px-5 gap-3 lg:gap-4">
      <Link href="/" className="flex items-center gap-3 shrink-0" aria-label="Prompt.Monster ana sayfa">
      ...
      <div className="ml-auto flex items-center gap-2 shrink-0">
        ...
        <button
          ...
          onClick={onSave}
          disabled={saving}
          title={user ? "Projeyi kütüphanene kaydet" : "Kaydetmek için giriş yap"}
          className={cx(
        ...
        {user ? (
              ...
              <div role="menu" className="absolute right-0 mt-2 w-56 rounded-xl bg-ink-800 border border-ink-600 shadow-xl p-1.5 z-50">
                ...
                <form action="/auth/signout" method="post">
                  <button type="submit" role="menuitem" className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-[13px] text-zinc-300 hover:bg-ink-600">
                    <LogOut className="w-4 h-4" aria-hidden /> Çıkış yap
                  </button>
                </form>
              </div>
            )}
          ...
          <Link href="/login?next=/studio" className="h-8 px-3 rounded-full bg-white text-black text-[12px] font-bold flex items-center gap-1.5">
            <LogIn className="w-3.5 h-3.5" aria-hidden /> Giriş
          </Link>
        )}
      </div>
    </nav>
